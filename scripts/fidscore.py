...
if __name__ == "__main__":
    ...
    inception_model = InceptionV3([3], normalize_input=False)
    ...
    mu, sigma = calculate_activation_statistics(images, inception_model, cuda=True, batch_size=50, dims=2048)
    # Load the precalculated training set statistics
    mu_train = np.load('mu_train.npy')
    sigma_train = np.load('sigma_train.npy')
    # Calculate the FID
    fid_value = calculate_frechet_distance(mu, sigma, mu_train, sigma_train)
    print('FID: ', fid_value)
